...
​		eureka：
​		eureka是什么？
​		eureka是`Netflix`的子模块之一，也是一个核心的模块，eureka里有2个组件，

1. 一个是EurekaServer(一个独立的项目) 这个是用于定位服务以实现中间层服务器的负载平衡和故障转移，

2. 另一个便是EurekaClient（我们的微服务） 它是用于与Server交互的，可以使得交互变得非常简单:只需要通过服务标识符即可拿到服务。
...
3. 与spring-cloud的关系：
   ...
   而**系统中的其他微服务**，使用 Eureka 的客户端连接到 Eureka Server并维持**心跳连接**。这样系统的维护人员就可以通过 Eureka Server 来监控系统中各个微服务是否正常运行。

   SpringCloud 的一些其他模块（比如Zuul，Hystrix）就可以通过 Eureka Server 来发现系统中的其他微服务，并执行相关的逻辑。
...
spring:
    application:
        name: server-discover-eureka

#eureka配置
eureka:
    server:
        enable-self-preservation: true  # 自我保护机制  15分钟 超过85%
...
#        healthCheckUrlPath: ${server.servletPath}/health
    ...
    client:
        serviceUrl:
            ...
            # eureka集群配置  cluster
#            defaultZone: http://eureka3001.com:3001/eureka,http://eureka3002.com:3001/eureka
        # 此应用为注册中心，false：不向注册中心注册自己[默认为true]。
#        当搭建集群时，需要注册自己
...
App中添加，注解启动 `@EnableEurekaServer` 注册中心的服务端
...
eureka服务端[微服务模块基本都注册到eureka中管理]项目里面加入以下配置：
...
spring:
    application:
        name: server-order #此实例注册到eureka服务端的name, 如果做集群。这个名字需要相同
...
eureka:
    ...
    instance:
        instance-id: order-0 #此实例注册到eureka服务端的唯一的实例ID
...
App中添加，注解启动 `@EnableEurekaClient`  ；将服务注册到Eureka中
...
###  eureka对比Zookeeper：
​		**Zookeeper**在设计的时候遵循的是**CP原则**，即一致性,Zookeeper会出现这样一种情况，当master节点因为网络故障与其他节点失去联系时剩余节点会重新进行leader选举，问题在于，选举leader的时间太长：30~120s，且选举期间整个Zookeeper集群是不可用的，这就导致在选举期间注册服务处于瘫痪状态，在云部署的环境下，因网络环境使Zookeeper集群失去master节点是较大概率发生的事情，虽然服务能够最终恢复，但是漫长的选举时间导致长期的服务注册不可用是不能容忍的。
 	

​		**Eureka**在设计的时候遵循的是**AP原则**，即可用性。Eureka各个节点（服务)是平等的， 没有主从之分，几个节点down掉不会影响正常工作，剩余的节点（服务） 依然可以提供注册与查询服务，而Eureka的客户端在向某个Eureka注册或发现连接失败，则会自动切换到其他节点，也就是说，只要有一台Eureka还在，就能注册可用（保证可用性）， 只不过查询到的信息不是最新的（不保证强一致），除此之外，Eureka还有自我保护机制，如果在15分钟内超过85%节点都没有正常心跳，那么eureka就认为客户端与注册中心出现了网络故障，此时会出现一下情况:
